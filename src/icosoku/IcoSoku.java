...
public class IcoSoku {
	...
	public boolean setPiece(int area, int piece, int orientation) {
		if (area < 0 || area > 19 || piece < 0 || piece > 19 || orientation < 0
				|| orientation > 2) {
			throw new IllegalArgumentException();
		}
		if (isAreaOccupied[area] || !isPieceAvailable[piece]) {
			return false;
		} else {
	...
	public boolean doable() {
		for (int i = 0; i < edgeNumbers.length; i++) {
			...
			if (pieceCountAtEdge(i) == 5) {
				...
				boolean doableWith3 = Math.ceil(differenceAtEdge / 3.0) > differenceAtPieces;
				if (doableWith3) {
					return false;
				}
			}
